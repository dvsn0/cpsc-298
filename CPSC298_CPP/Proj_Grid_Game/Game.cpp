...
Game::Game() : m_nGridRows(k_nGridRowsDefault), m_nGridColumns(k_nGridColumnsDefault) /* ?? add initializers for member variables m_iRowBoat, m_iColumnBoat, and m_nTorpedoes. ?? */
{
	...
	this->m_iRowBoat = 0;
	this->m_iColumnBoat = 0;
	this->m_nTorpedoes = k_nMaxTorpedoes;
	initializeGrid();
}
...
std::tuple<int, int>  Game::fireTorpedo()
{
	...
	this->m_nTorpedoes = m_nTorpedoes - 1;
...
void Game::showSunk()
{
	...
	this->a_cGrid[m_iRowBoat][m_iColumnBoat] = 'X';
	return;
}
...
std::tuple<int, int> Game::promptForGridCoord()
{
	...
	std::cout << "Please neter grid row number: " << std::endl;
