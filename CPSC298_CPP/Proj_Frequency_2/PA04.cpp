/// @file PA04.cpp
/// @brief Program calculating musical note frequencies.
/// @author David Sohn
...
double frequency(double v, double k){
    return (16.35 * pow(2, (v + (k / 12))));
}
...
int main(){
    ...
    for (int nu = 0; nu < k_nOctaves; nu++){
        for (int k = 0; k < k_nHalfTones; k++){
            ...
            double f = frequency(nu, k);
            // Calculate wavelength (dWavelengthCentimeters)
            double dWavelengthCentimeters = wavelength(f);
